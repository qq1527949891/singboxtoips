...
configure_tc_service_with_params() {
    ...
    # 构建 tc 脚本的内容
    # 使用 here-document (cat <<EOF) 来更清晰地写入脚本内容，避免复杂的字符串拼接
...
if ! command -v tc &> /dev/null; then
    echo "\$(date) - 错误：tc 命令未找到，无法应用带宽限制。"
    exit 1
fi
...
# 动态插入网络信息到脚本中
declare -g -A PUBLIC_IP_TO_PRIVATE_IP_MAP=(
EOF
    # 动态插入关联数组内容，每个键值对占一行
    for pub_ip in "${!PUBLIC_IP_TO_PRIVATE_IP_MAP[@]}"; do
        echo "[\"$pub_ip\"]=\"${PUBLIC_IP_TO_PRIVATE_IP_MAP[$pub_ip]}\"" | sudo tee -a "$TC_SCRIPT_PATH" > /dev/null
    done
...
for private_ip in "\${!PRIVATE_IP_TO_MARK_MAP[@]}"; do
    ...
    if [ -z "\$interface_name" ] || [ -z "\$routing_mark" ]; then
        echo "\$(date) - 警告：无法为私网IP \$private_ip 找到网卡或路由标记，跳过。"
        continue
    fi

    echo "\$(date) - 正在为网卡 \$interface_name (路由标记 \$routing_mark) 配置带宽限制..."

    # 1. 为主网卡添加主 qdisc (htb)，使用 replace 避免失败
    if tc qdisc show dev "\$interface_name" | grep -q "qdisc htb"; then
        tc qdisc replace dev "\$interface_name" root handle 1: htb default 12
    else
        tc qdisc add dev "\$interface_name" root handle 1: htb default 12
    fi
    if [ \$? -ne 0 ]; then
        echo "\$(date) - 错误：添加/替换主 qdisc 失败。请手动检查 tc qdisc show dev \$interface_name。"
        continue
    fi

    # 2. 添加一个主 class 来承载所有子类
    tc class replace dev "\$interface_name" parent 1: classid 1:1 htb rate 1000Mbps
    if [ \$? -ne 0 ]; then
        echo "\$(date) - 错误：添加/替换主 class 1:1 失败。"
        continue
    fi

    # 3. 为每个协议添加一个子类，使用 replace 避免失败
    protocol_class_id=\$((10 + routing_mark))
    tc class replace dev "\$interface_name" parent 1:1 classid 1:"\$protocol_class_id" htb rate "\${BANDWIDTH_LIMIT_MBPS}Mbps" ceil "\${BANDWIDTH_LIMIT_MBPS}Mbps"
    if [ \$? -ne 0 ]; then
        echo "\$(date) - 错误：添加/替换协议子类 1:\$protocol_class_id 失败。"
        continue
    fi
    ...
    tc filter add dev "\$interface_name" protocol ip parent 1: prio 1 handle "\$routing_mark" fw flowid 1:"\$protocol_class_id"
    if [ \$? -ne 0 ]; then
        echo "\$(date) - 错误：添加 filter 失败。请手动检查 iptables 和 tc 配置。"
        continue
    fi

    # 5. 为每个 class 添加一个子 qdisc (sfq)，使用 replace 避免失败
    tc qdisc replace dev "\$interface_name" parent 1:"\$protocol_class_id" sfq perturb 10
    if [ \$? -ne 0 ]; then
        echo "\$(date) - 错误：添加/替换子 qdisc 失败。"
        continue
    fi
done
